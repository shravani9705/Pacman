...
def initializeK(X, K, birthPositions):
    ...
    for k in range(len(birthPositions), K):
        D2 = [min([abs(c[0]-x[0])+abs(c[1]-x[1]) for c in C]) for x in X]
        D2sum = sum(D2)
        #print(C)
        if sum(D2) == 0: return C
        probs = [d/D2sum for d in D2]
        cumprobs = []
        for i in range(len(probs)):
            cumsum1 = probs[i]
            for j in range(i):
                cumsum1 += probs[j]
            cumprobs.append(cumsum1)
        import random
...
class MyAgent(Agent):
    ...
    def registerInitialState(self, state):
        ...
        if self.index==0:
            ...
            if MyAgent.pacmanGo2Destination[0] == 0: return
