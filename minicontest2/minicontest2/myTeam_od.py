  ...
  def chooseAction(self, gameState):
    ...
    if self.mode == "AlphaBetaAgent":
      if self.pastIsinEnemyRegion == True and self.currentIsinEnemyRegion == False:
        if gameState.getAgentPosition(self.index) == gameState.getInitialAgentPosition(self.index):
          ...
          self.init = True
          return self.chooseAction(gameState)
      elif gameState.getAgentState(self.index).numCarrying > min(minGhostDistance - 3, 3) \
    ...
    if self.mode == "gotoAnotherPlaceAgent":
      if invaders:
        ...
        self.init = True
        return self.chooseAction(gameState)
      elif minGhostDistance > 8 and not invaders:
        self.mode = "AlphaBetaAgent"
        self.init = True
        return self.chooseAction(gameState)
      if self.init == True:
...
class gotoAnotherPlaceAgent(DummyAgent):
  ...
  def getWeights(self, gameState, action):
    return {'numInvaders': -1000, 'onDefense': 100, 'distanceToFood': 10, 'invaderDistance': -60, 'stop': -100, 'reverse': -2, 'borderDist': 0, 'minGhostDist': 100}
...
class goHomeAgent(InvadeAgent):
  ...
  def getWeights(self, gameState):
    return {'distanceToHome': 100, 'rDistanceToGhost': 50, 'getEaten': 10000, 'IamHome!!': 500, 'goHomeTime': 100,\
            'distanceToCapsule': 200, 'goToEatCapsule':300}
...
class ChooseInvadePositionAgent(InvadeAgent):
  ...
  def getWeights(self, gameState, action):
    return {'successorScore': 100, 'distanceToFood': -5, 'rDistanceToGhost': 100}
...
class LureAgent(DummyAgent):
  ...
  def getWeights(self, gameState, action):
    return {'numInvaders': -1000, 'onDefense': 0, 'invaderDistance': -10, 'stop': 10, 'reverse': -2, 'borderDist': 30, \
            'getEaten': 10000, 'luring': 50, 'rDistanceToGhost': 100}
